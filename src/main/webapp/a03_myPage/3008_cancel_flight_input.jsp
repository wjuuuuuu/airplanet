	...
	<%@ include file="/header.jsp" %>
		...
		<div class="row justify-content-center">
			...
			<div class="col-md-10 .offset-md-3">
				...
				<div class="form-check">
					...
					<label style="font-size:30px;margin-top:10px;"class="form-check-label" for="flexCheckChecked">
						<b>탑승객1  ${fn:substring(fareData[0].korname,0,3)}</b>
					</label>
				</div>
					<div style="margin-left:60px;margin-top:10px;">
						<h4>${fareData[0].departAirport} - ${fareData[0].arriveAirport}</h4>
					</div>
					<div style="margin-left:60px;margin-top:12px;">
						<p style="font-size:25px;"><b>출발날짜 및 시간</b></p>
						<p style="font-size:20px;">${fn:substring(fareData[0].departDate,0,4)}년 ${fn:substring(fareData[0].departDate,5,7)}월 ${fn:substring(fareData[0].departDate,8,10)}일 ${fn:substring(fareData[0].departDate,11,13)}시 ${fn:substring(fareData[0].departDate,14,16)}분</p>
			...
			<div class="col-md-6 .offset-md-3">
					<div class="form-check">
						<input style="zoom:2.0;margin-left:0px;margin-top:10px;" class="form-check-input" type="checkbox" name="cancelP" value="${fn:substring(fareData[0].korname,4,7)}" id="flexCheckDefault" onclick='getCheckedCnt()'>
						  <label style="font-size:30px;margin-top:10px;"class="form-check-label" for="flexCheckDefault">
						    <b>탑승객2 ${fn:substring(fareData[0].korname,4,7)}</b>
						  </label>
					</div>
					<div style="margin-left:60px;margin-top:10px;">
						<h4>${fareData[0].departAirport} - ${fareData[0].arriveAirport}</h4>
